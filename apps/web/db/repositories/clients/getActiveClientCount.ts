...
export const getActiveClientCount = async (): Promise<ActiveClientCountResponse> => {
  try {
    ...
    if (typeof activeClientCount !== 'number' || activeClientCount < 0) {
      if (typeof activeClientCount !== 'number' || activeClientCount < 0) {
        throw new InvalidDataError('Invalid count value received from database.', {
          activeClientCount,
        });
      }
    }

    logger.debug({ activeClientCount }, 'Retrieved active client count successfully.');
    ...
    return handleError(error, 'getActiveClientCount') as ActiveClientCountResponse;
  }
};
