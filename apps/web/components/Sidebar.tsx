...
const Sidebar = ({ role }: { role: Roles }) => {
  ...
  return (
    <aside
      className={cn(
        'h-screen bg-white dark:bg-background border-r flex flex-col justify-between transition-all duration-300',
    ...
    >
      <div>
        ...
        >
          {!isCollapsed && <Logo width={100} height={100} />}
          <button
        ...
        <nav className="flex flex-col items-center">
          {allowedLinks.map(({ path, label, Icon }) => (
            <div key={path} className="flex flex-row items-center w-full">
              <ActiveMenu pathname={path} activePath={pathname} isCollapsed={isCollapsed} />
              <LinkButton
                variant={'ghost'}
                size={'lg'}
                href={path}
                className={cn(
                  ...
                  isCollapsed ? 'justify-center px-4' : 'justify-start px-12',
                  pathname.startsWith(path) ? 'text-purple-600' : 'text-textPrimary'
                )}
                title={isCollapsed ? label : undefined}
              >
                <Icon className="w-5 h-5 flex-shrink-0" />
                {!isCollapsed && <p className="w-28">{label}</p>}
              </LinkButton>
            </div>
          ))}
        </nav>
