...
const editTaskSchema = z.object({
  ...
  timeForTask: z
    ...
    .refine(
      ...
      {
        message:
          "Invalid format for Original Estimate. Examples: '4d', '5h 30m', '60m', '3w'",
      },
    ),
  overTime: z
    .string()
    ...
    startTransition(async () => {
      try {
        ...
        if (data.success) {
          ...
          setFiles((prevFiles) =>
            prevFiles.filter((file) => file.file.id !== fileId),
          );
          toast({
  ...
  const onSubmit = async (data: EditTaskFormValues) => {
    try {
      ...
      const formattedData = {
        ...trimmedData,
        id: task.id,
        dueDate: trimmedData.dueDate ?? null,
